...
================Loon==============
[Script]
cron "33 0-23/1 * 6 *" script-path=https://raw.githubusercontent.com/Wenmoux/scripts/master/jd/jd_618redpacket.js tag=翻翻乐
            ...
            if (cookie) {
                ...
                if (leftTime != 0) {
                   ...
                   await open("gambleOpenReward")               
                    ...
                    if ($.canDraw) {
                     let dd =   await open("gambleObtainReward", $.reward.rewardType)
                     let type = dd.prizeType ===4 ?"  ￥  ":"  🧧  "
                   //   $.message += "当前获得：" +type+ $.reward.rewardValue + "\n"
                     if(dd.prizeType != 4){
                     console.log("当前奖励类型不是￥,不进行提现")  
                     }else{
