...
function jd(){
    ...
    rm -rf /acoolbook /scripts/jd_*
    git clone https://github.com/acoolbook/scripts.git /acoolbook
    cp /acoolbook/jddj_cookie.js /scripts/jddj_cookie.js
    # 拷贝脚本
    for jsname in $(find /acoolbook -name "*.js" | grep -vE "\/backup\/"); do cp ${jsname} /scripts/jd_${jsname##*/}; done
    # 匹配js脚本中的cron设置定时任务
    for jsname in $(find /acoolbook -name "*.js" | grep -vE "\/backup\/"); do
        jsnamecron="$(cat $jsname | grep -oE "/?/?cron \".*\"" | cut -d\" -f2)"
        test -z "$jsnamecron" || echo "$jsnamecron node /scripts/jd_${jsname##*/} >> /scripts/logs/jd_${jsname##*/}.log 2>&1" >> /scripts/docker/merged_list_file.sh
    done
}
#cp /acoolbook/jddj_cookie.js /scripts/jddj_cookie.js
...
echo "10 2,15 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_jd_mc.js >> /scripts/logs/jd_jd_mc.js.log 2>&1" >> /scripts/docker/merged_list_file.sh
echo "20 4,17 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_jd_mc.js >> /scripts/logs/jd_jd_mc.js.log 2>&1" >> /scripts/docker/merged_list_file.sh
echo "30 6,19 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_jd_mc.js >> /scripts/logs/jd_jd_mc.js.log 2>&1" >> /scripts/docker/merged_list_file.sh
echo "40 8,21 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_jd_mc.js >> /scripts/logs/jd_jd_mc.js.log 2>&1" >> /scripts/docker/merged_list_file.sh
echo "50 10,23 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_jd_mc.js >> /scripts/logs/jd_jd_mc.js.log 2>&1" >> /scripts/docker/merged_list_file.sh
