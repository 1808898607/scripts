...
if ! [ `grep -c "$FIND_STR" $FIND_FILE` -ne '0' ];then
      sed -i 's/\/scripts\/logs\/auto_help_export\.log/\/scripts\/logs\/auto_help_export\.log \&\& \. \/scripts\/docker\/export_jd_acoolbook\.sh/g' $FIND_FILE
      ## 更新定时任务列表
      crontab $FIND_FILE
fi
...
if ! [ `grep -c "$FIND_STR" $CRZAY_JOY_COIN_FILE` -ne '0' ];then
      sed -i 's/node \/scripts\/jd_crazy_joy_coin\.js/\. \/scripts\/docker\/export_jd_cookies_acoolbook\.sh \&\& node \/scripts\/jd_crazy_joy_coin\.js/g' $CRZAY_JOY_COIN_FILE
